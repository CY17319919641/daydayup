...
import com.daydayup.adapter.QuestionPagerAdapter
...
// 隐藏系统UI
    ...
    private fun updateTabStyle(position: Int, isSelected: Boolean) {
        if (position < tabViews.size) {
            ...
            if (isSelected) {
                // 选中状态
                textView.setTextColor(android.graphics.Color.parseColor("#B0704A"))
                textView.setBackgroundResource(R.drawable.bg_subject2)
            } else {
                // 未选中状态
                textView.setTextColor(android.graphics.Color.parseColor("#007F4A"))
